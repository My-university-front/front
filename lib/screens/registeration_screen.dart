...
class _RegisterationScreenState extends State<RegisterationScreen> {
  ...
  checkValidation(BuildContext context) async {
    if (email.length == 0) {
      _showSnackBar(context, 'fill email');
      return;
    }
    if (firstName.length == 0) {
      _showSnackBar(context, 'fill first name');
      return;
    }
    if (lastName.length == 0) {
      _showSnackBar(context, 'fill last name');
      return;
    }
    if (sid.length == 0) {
      _showSnackBar(context, 'fill student ID');
      return;
    }
    if (password.length == 0) {
      _showSnackBar(context, 'fill password');
      return;
    }
    count++;
    if (count > 1) {
  ...
  post(String url, BuildContext context) async {
    ...
    try {
      ...
      if (result.statusCode == 201) {
        ...
        resetCounter();
        _showSnackBar(context, 'Please insert an academic email');
      } else if (result.statusCode == 500) {
        setState(() {
          showSpinner = false;
        });
        resetCounter();
        _showSnackBar(context, 'This email is used by another user');
      } else {
        setState(() {
          showSpinner = false;
        });
      ...
      showSpinner = false;
      resetCounter();
      _showSnackBar(context, 'There is a problem with host');
      print("My Error: $e");
    }
  }
